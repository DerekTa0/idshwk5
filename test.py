...
'''
#预测测试集
def predict(model,test_target,test_str):
...
if __name__ =="__main__":
    ...
    train_target=train_target.reshape(train_data.shape[0],)
    classifier=LogisticRegression()
    classifier.fit(train_data,train_target)
    test_target=classifier.predict(test_data)
    predict(None,test_target,test_str)
